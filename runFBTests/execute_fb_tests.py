...
class FBWrapper:
    ...
    def clean_temp_folder(build=False):
        """
    ...
    def start_tests(version=self.version):
        """
    ...
    def prepare_builds(self, builds):
        """
        	...
        	try:
                ...
                for section in config.sections():
                    ...
                    if not self.version or version == self.version:
                        ...
                        # Run the build(s)
                        if not opt.binary:
                            ret = prepare_builds(version, builds)
                        else:
                		    start_tests(version)
        	    ...
        	    print "[Error] Could not download 'test-bot.config' from '" + self.serverpath + "'"
